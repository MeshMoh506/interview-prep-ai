...
// }
// lib/features/interview/pages/interview_history_page.dart
// Shows all past interviews with score, grade, role, and date.
// Tapping an interview shows its full feedback.
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:go_router/go_router.dart';
import '../../../core/theme/app_colors.dart';
        ...
        SafeArea(
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 4),
              Text(isAr ? 'سجل المقابلات' : 'Interview History',
                  style: const TextStyle(
            ...
            error: (e, _) => Center(
                child: Column(
                  ...
                  const Icon(Icons.error_outline,
                      size: 48, color: AppColors.rose),
                  const SizedBox(height: 12),
                  Text(isAr ? 'فشل تحميل السجل' : 'Failed to load history',
                      style: const TextStyle(color: Colors.grey)),
                  const SizedBox(height: 12),
                  ElevatedButton(
                      onPressed: () => ref.invalidate(interviewHistoryProvider),
                ...
                return Center(
                    child: Column(
                      ...
                      ElevatedButton.icon(
                          ...
                          label: Text(isAr ? 'ابدأ الآن' : 'Start Now'),
                          style: ElevatedButton.styleFrom(
                              backgroundColor: AppColors.violet,
                              foregroundColor: Colors.white,
                              padding: const EdgeInsets.symmetric(
...
class _HistoryCard extends StatelessWidget {
  ...
  final bool isDark, isAr;
  const _HistoryCard(
      {required this.interview, required this.isDark, required this.isAr});

  Color get _scoreColor {
    final score = (interview['score'] as num?)?.toDouble() ?? 0;
  ...
  Widget build(BuildContext context) {
    final score = (interview['score'] as num?)?.toDouble() ?? 0;
    final grade = interview['grade']?.toString() ?? '';
    final rec = interview['recommendation']?.toString() ?? '';
    final role = interview['job_role']?.toString() ?? '—';
    final diff = interview['difficulty']?.toString();
    final status = interview['status']?.toString() ?? '';
    final date = _formatDate(interview['started_at']?.toString());
    final msgCount = (interview['message_count'] as num?)?.toInt() ?? 0;
        ...
        child: Row(children: [
          ...
          SizedBox(
              ...
              child: Stack(fit: StackFit.expand, children: [
                CircularProgressIndicator(
                  value: isCompleted ? (score / 100).clamp(0.0, 1.0) : null,
                  strokeWidth: 5,
                  color: _scoreColor,
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  _Chip(_difficultyLabel(diff), AppColors.violet),
                  const SizedBox(width: 6),
                  _Chip(date, Colors.grey),
                  if (msgCount > 0) ...[
                    const SizedBox(width: 6),
                    _Chip('${msgCount ~/ 2} Q', AppColors.cyan),
                  ],
                ]),
                if (isCompleted && rec.isNotEmpty) ...[
                  const SizedBox(height: 4),
                  Text(isAr ? _recAr(rec) : rec,
                      style: TextStyle(
                          color: _recColor(rec),
                          fontSize: 11,
                          fontWeight: FontWeight.w700)),
                ...
                if (!isCompleted) ...[
                  const SizedBox(height: 4),
                  Text(isAr ? 'غير مكتمل' : 'Incomplete',
  ...
  Color _recColor(String rec) {
    ...
    if (r.contains('hire')) return AppColors.emerald;
    if (r.contains('consider')) return AppColors.cyan;
    if (r.contains('work')) return AppColors.amber;
    return AppColors.rose;
  }

  String _recAr(String rec) {
    final r = rec.toLowerCase();
    if (r.contains('hire')) return 'جاهز للتوظيف ✅';
    if (r.contains('strong consider')) return 'يستحق التقديم';
    if (r.contains('consider')) return 'يحتاج تطوير';
    if (r.contains('work')) return 'يحتاج عملاً';
    return 'غير جاهز';
  }
}
